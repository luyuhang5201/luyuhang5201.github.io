			//音乐添加
			var playlist=1;
			   $.get("/admin/html/value.php",function(data){
			playlist=data.music;   
(function ($){
    var isShowNotification = false, isInitMarquee = true, shuffleArray = [], shuffleIndex, autoClearTimer, 
    autoShowTimer, isFirstPlay = localStorage.qplayer == undefined ? true : false, isShuffle = localStorage.qplayer == undefined ? false : localStorage.qplayer === 'true' ? true : false;
    ...
    /*关闭弹出框框*/
    $("#boox_gg").click(function(){
    $("#boox_gg").hide();
    play();
    $('#QPlayer .ssBtn').click();//唤出播放器
    });
    ...
    $('#QPlayer .liebiao,#QPlayer .liebiao').on('click', function ()
    {
        ...
        {
            pl.css({
                "max-height" : "360px", "transition" : "max-height .5s ease"
            });
        ...
        {
            pl.css({
                "max-height" : "0px", "transition" : "max-height .5s ease"
            });
            pl.css("border", "0");
            pl.removeClass('go');
        }
...
{
    $('.marquee').marquee({
        duration : 15000, gap : 50, delayBeforeStart : 0, direction : 'left', duplicated : true
    });
}
...
function showNotification(info)
{
    ...
    {
        $('.qplayer-notification').css({
            "width" : width, "transition" : "all .7s ease"
        });
...
function closeNotification()
{
    ...
    $('.qplayer-notification').css({
        "width" : "50px", "transition" : "all .7s ease"
    });
...
function showTips(div, info, func)
{
    ...
    $('.qplayer_tips').css({
        "top" : $(div).offset().top - box_height - 30 - 15, "left" : $(div).offset().left - 28
    });
    $('.qplayer_tips').css({
        "height" : box_height, "transition" : "all .5s ease-in-out"
    });
    $('.qplayer_tips .info').delay(500).fadeIn();
    $('.tips_arrow').css({
        "border-width" : "15px", "transition" : "all .5s ease-in-out"
    });
    $('.tips_button').css({
        "height" : "30px", "transition" : "all .5s ease-in-out"
    });
    if (func != undefined) {
...
function removeTips()
{
    ...
    $('.tips_arrow').css({
        "border-width" : "0", "transition" : "all .5s ease-in-out"
    });
    $('.tips_button').css({
